 ...
 */
const BlogGallery = ({defaultView = 'grid' }) => {
...
const ProjectCard = ({ project, viewType }: { project: Project; viewType: string }) => {
  const { title, imageUrl, date, slug } = project;
  ...
  return (
    <div className="group bg-white overflow-hidden border border-gray-200 rounded">
      <Link href={`/blog/${slug}`} className="block" >
        ...
        <h3 className="text-base md:text-lg lg:text-xl font-semibold mb-2 text-gray-800">{title}</h3>
          <p className="text-gray-600 text-sm mb-3">{date}</p>
